...
Definir a estrutura, governança e operação do repositório canônico de memórias operacionais do Maui: `vault-maui/memorias/`. O Memory Store runtime é o local onde decisões, marcos, preferências e artefatos com valor futuro serão preservados de forma seletiva, sanitizada e rastreável quando o Maui operar.

Enquanto o Maui está em implementação, `vault-maui/project-memories/` guarda memória de projeto/gestação e não substitui o Memory Store runtime.

---

## 1. Princípio fundamental

**Os arquivos markdown em `vault-maui/memorias/` serão a fonte de verdade do runtime.** Durante a implementação, arquivos em `vault-maui/project-memories/` são fonte histórica de projeto.
...
- Se índice ausente (pré-P0.3): scan manual de `vault-maui/memorias/` com grep por frontmatter quando houver runtime; para histórico de projeto, scan manual de `vault-maui/project-memories/`.
