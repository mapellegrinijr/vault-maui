...
- HEAD de referência na coleta: `1d875e367f93645402b9eafdcba5e0f657d5a246` (`docs(project): alinhar roadmap e core à separação project vs runtime`).
- Working tree na coleta: limpo.
...
3. `vault-maui/handoffs/2026-05-10-handoff-sessao-pacote-status-context.md` — handoff da sessão do pacote manual-first.
4. `vault-maui/exec-reports/submitted/2026-05-10-p0-x-pacote-manual-first-status-context-handoff.md` — exec-report de governança do pacote.
...
8. `vault-maui/project/roadmap/roadmap-desenvolvimento-maui-v1-0.md` — mapa de destino; confirmar execução por evidência, não pelo roadmap isolado.
...
- Separação Project vs Runtime aplicada por `f8b5120` e complementada por `9c76d62` e `1d875e3`.
- Status corrente do projeto vive em `status-project/`; memórias de projeto vivem em `project-memories/`.
- `memorias/` e `status/` são pastas runtime reservadas, ainda sem uso operacional.
...
- Fato: HEAD de referência `1d875e3` estava limpo na coleta.
